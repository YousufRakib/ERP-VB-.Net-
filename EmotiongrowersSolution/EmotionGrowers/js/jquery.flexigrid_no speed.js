...
(function ($) {
    ...
    $.addFlex = function (t, p) {
        ...
        p = $.extend({
            ...
            rpOptions: [10, 15, 20, 25, 40, 50, 75, 100],
            title: false,
        ...
        function GetPages() {
            ...
            if (page.indexOf('Page=Order') >= 0 || page.indexOf('Page=ConfirmedPOs') >= 0 || page.indexOf('Page=PendingPOs') >= 0 || page.indexOf('Page=ManualPO') >= 0 || page.indexOf('Page=Incoming') >= 0 || page.indexOf('Page=PO') >= 0 || page.indexOf('Page=Grower') >= 0 || page.indexOf('Page=Accounts') >= 0 || page.indexOf('Page=Credits') >= 0 || page.indexOf('Page=Sales') >= 0) {
                return true;
            }
            else {
                return false;
        ...
        function GetTotals() {
            ...
            var page = window.location.href;
            var val18 = 0, val19 = 0, val20 = 0, val21 = 0;
            ...
            if (page.indexOf('Page=Order') >= 0) {
                ...
                if (sel.length > 0) {
                    for (var x = 0; x < sel.length - 1; x++) {
                        ...
                        if (style == "table-row") {
                            ...
                            val18 += parseInt($(tds[19]).find('div').text(), 10);

                            val19 += parseFloat($(tds[20]).find('div').text());

                            val20 += parseFloat($(tds[21]).find('div').text().substring(2).replace(',', ""));

                            //val21 += parseFloat($(tds[22]).find('div').text().split("%")[0]);

                            TotalFOBAmount += parseFloat($(tds[27]).find('div').text());

                            TotalInvCost += parseFloat($(tds[28]).find('div').text());
                        }
                ...
                if (TotalFOBAmount != 0)
                    val21 = ((TotalFOBAmount - TotalInvCost) / TotalFOBAmount) * 100


                val19 = stringFormatFBE(val19)
                val20 = "$ " + stringFormatInv(val20)
                val21 = stringFormatGPM(val21) + "%"

                $("#fgrdOrderHeader").parent(".bDiv").siblings(".hDiv").find("table tr th:nth-child(20) div.totalRowFontStyle").text(val18);
                $("#fgrdOrderHeader").parent(".bDiv").siblings(".hDiv").find("table tr th:nth-child(21) div.totalRowFontStyle").text(val19);
                $("#fgrdOrderHeader").parent(".bDiv").siblings(".hDiv").find("table tr th:nth-child(22) div.totalRowFontStyle").text(val20);
                $("#fgrdOrderHeader").parent(".bDiv").siblings(".hDiv").find("table tr th:nth-child(23) div.totalRowFontStyle").text(val21);

                //$("#DivOrderHeader div.hDivBox").find("th:eq(18) div").html("Pcs<br><div class='totalRowFontStyle'>" + val18 )
                //$("#DivOrderHeader div.hDivBox").find("th:eq(19) div").html("FBE<br><div class='totalRowFontStyle'>" + val19)
                //$("#DivOrderHeader div.hDivBox").find("th:eq(20) div").html("$Inv.Amount<br><div class='totalRowFontStyle'>" + val20 )
                //$("#DivOrderHeader div.hDivBox").find("th:eq(21) div").html("%GPM<br><div class='totalRowFontStyle'>" + val21)
                //alert(val18 + "," + val19 + "," + val20 + "," + val21);
            }
        ...
        function GetIncomingTotals() {
            ...
            if (page.indexOf('Page=Incoming') >= 0) {
                ...
                if (sel.length > 0) {
                    for (var x = 0; x < sel.length - 1; x++) {
                        ...
                        if (style == "table-row") {
                            ...
                            var tds = jQuery(sel[x]).children('td');
                            Boxes += parseInt($(tds[5]).find('div').text(), 10);
                            FBE += parseFloat($(tds[6]).find('div').text());
                            Amount += parseFloat($(tds[7]).find('div').text());
                            Scanned += parseFloat($(tds[13]).find('div').text(), 10);
                            Missing += parseFloat($(tds[14]).find('div').text(), 10);
                            Weight += parseFloat($(tds[15]).find('div').text());
                            DWeight += parseFloat($(tds[16]).find('div').text());
                            MPF += parseFloat($(tds[17]).find('div').text());
                            Duties += parseFloat($(tds[18]).find('div').text());
                        }
                ...
                $('#DivUserHeaders #row0 td:nth-child(14) div').text(Scanned);
                $('#DivUserHeaders #row0 td:nth-child(15) div').text(Missing);
                $('#DivUserHeaders #row0 td:nth-child(16) div').text(Weight);
                $('#DivUserHeaders #row0 td:nth-child(17) div').text(DWeight);
                $('#DivUserHeaders #row0 td:nth-child(18) div').text(MPF);
                $('#DivUserHeaders #row0 td:nth-child(19) div').text(Duties);
            }
        }
        ...
        var g = {
            ...
            addData: function (data) { //parse data
                ...
                if (p.total == 0) {
                    ...
                    if (this.bDiv.firstChild.id == "fgrdInventoryUsersHeaders" || this.bDiv.firstChild.id == "fgrdOrderHeader") {
            ...
            buildpager: function () { //rebuild pager based on new properties
                ...
                if (this.pDiv) {
                    if (this.bDiv.firstChild.id != "fgrdManualPOHeader" && this.bDiv.firstChild.id != "fgrdInventoryDetailbyAWB" && this.bDiv.firstChild.id != "fgrdInventoryUserFarmDetails" && this.bDiv.firstChild.id != "fgrdBoxDetails" && this.bDiv.firstChild.id != "fgrdArmelliniXMLLogs" && this.bDiv.firstChild.id != "fgrdARINVSViewChecksHeader" && this.bDiv.firstChild.id != "fgrdARINVSViewChecksDetails" && page.indexOf("Default.aspx") < 0) {
                        this.domElements.pPageStat.html(stat).css('font-weight', 'normal').css('font-size', '11px').css('color', 'black').css('top', '5px').css('margin-left', '-28px');
                    }
                    else {
            ...
            populate: function () { //get latest data
                ...
                if (this.pDiv) {
                    if (this.bDiv.firstChild.id != "fgrdManualPOHeader" && this.bDiv.firstChild.id != "fgrdInventoryDetailbyAWB" && this.bDiv.firstChild.id != "fgrdInventoryUserFarmDetails" && this.bDiv.firstChild.id != "fgrdBoxDetails" && page.indexOf("Default.aspx") < 0) {
                        this.domElements.pPageStat.html(p.procmsg).css('font-weight', 'bold').css('font-size', '15px').css('color', 'red').css('top', '0').css('margin-left', '-28px');
                    }
                    else {
                    ...
                    $("#" + this.bDiv.firstChild.id + " tr").each(function (index) {
                        ...
                        if (p.qtype == 'WH') {
                            ...
                            if (id.indexOf(value) != -1) {
                                ...
                                if ($(this).attr('id') === "row0" && p.query != "" && (page.indexOf('Page=Grower') >= 0 || page.indexOf('Page=Incoming') >= 0 || page.indexOf('Page=ConfirmedPOs') >= 0 || page.indexOf('Page=Accounts') >= 0)) {
                                    $row.show();
                                }
                                else
                                    $row.hide();
        ...
        if (p.usepager) {
            ...
            if (p.searchitems) {
                ...
                if (page.indexOf('Page=Sales') >= 0) {
                    $(g.sDiv).append("<div class='sDiv2'>Quick Search <input type='text' size='30' name='q' class='qsbox' /><select name='lstsearch'>  <option value='1'> Equals </option>  <option value='2'> Not equals </option></select><select name='qtype'>" + sopt + "</select> <input style='height:24px' type='button' value='Search' /><input style='height:24px' type='button' value='Clear' /><input style='height:24px;display:none;' type='button' id='ExportCSV'  value='Export' class='ExportCSV' /></div>");
                }
                else {
